...
const BODY_TEXT_WEIGHT_VALUES: Record<string, string> = {
  normal: "400",
  medium: "500",
  semibold: "600",
};
...
export function buildThemeOverrides(settings: SiteSettings): Record<string, string> {
  ...
  // Body text weight: exposed so "the text looks faint" is fixable from the
  // admin rather than in code. Consumed by the .text-ink-muted rule in
  // globals.css; "normal" is the shipped look.
  overrides["body-text-weight"] = BODY_TEXT_WEIGHT_VALUES[settings.body_text_weight ?? "normal"];
  return overrides;
}
